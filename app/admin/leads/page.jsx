...
export default function AdminLeadsPage() {
  ...
  const filteredLeads = leads.filter(lead => {
    ...
    return (
      ...
      lead.broker?.name?.toLowerCase().includes(search)
    );
  });
  ...
  return (
    <div className="min-h-screen pt-24 pb-12">
      <div className="container-x">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-bold mb-2">All Leads - Admin Dashboard</h1>
          <p className="text-neutral-400">View and manage all leads across the platform</p>
        </div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1">
              ...
              <input
                ...
                placeholder="Search by name, phone, property, or broker..."
                className="w-full px-3 py-2 bg-neutral-800 border border-white/10 rounded focus-ring"
              />
            </div>
          </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-neutral-800 border-b border-white/10">
                    ...
                    <th className="text-left px-4 py-3 text-sm font-medium text-neutral-300">Broker</th>
                  ...
                  {filteredLeads.map((lead) => (
                    <tr key={lead.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-4 py-4">
                        {lead.broker ? (
                          <div>
                            <div className="font-medium text-white">{lead.broker.name}</div>
                            {lead.broker.phone && (
                              <div className="text-sm text-neutral-400">{lead.broker.phone}</div>
                            )}
                          </div>
                        ) : (
                          <span className="text-neutral-500">No broker</span>
                        )}
                      </td>
                      <td className="px-4 py-4">
                        <select
